...
// Define FIFO offsets if not already defined
...
#define XPAR_AXI_FIFO_0_BASEADDR 0x43C00000 // Same as Conv
#endif
...
namespace ML
{
    ...
    void DenseLayer::computeQuantized(const LayerData &dataIn) const
    {
        ...
        if (!dense_calibration_loaded)
        {
            ...
            if (!found)
            {
                logError("Could not find dense calibration stats file");
        ...
        if (use_dense_layer_specific_calibration || outputSize == 200)
        {
            ...
            zi = static_cast<i8>(std::max(static_cast<int>(QUANT_MIN), std::min(static_cast<int>(QUANT_MAX), static_cast<int>(zi))));
    ...
    {
#ifdef ZEDBOARD
        // Check calibration (Dense uses its own flag)
        if (!dense_calibration_loaded)
        {
            // Attempt to load default if not loaded?
            // Reuse logic from computeQuantized section 1?
            // For brevity, assume it's loaded or fail.
            // Actually, best to try loading it same as computeQuantized
            // But strict error here:
            if (!loadDenseCalibrationStats("calibration_stats.json") && 
                !loadDenseCalibrationStats("calibration_stats_int4.json") && 
                !loadDenseCalibrationStats("calibration_stats_int2.json")) { // Try generic
                 logError("Calibration data not loaded for Accelerated Dense Inference");
                 return;
            }
        }
        ...
        // 1. Determine Input Quantization Params (Si, zi)
        fp32 Si;
        i8 zi;
        ...
        if (use_dense_layer_specific_calibration || outputSize == 200)
        {
            ...
            Si = QUANT_RANGE / input_range;
            fp32 zero_point_float = -Si * (input_min + input_max) / 2.0f;
            zi = static_cast<i8>(std::max(static_cast<fp32>(QUANT_MIN), std::min(static_cast<fp32>(QUANT_MAX), std::round(zero_point_float))));
        }
        else
        {
            ...
            Si = input_stats_it->second.Si;
            zi = input_stats_it->second.zi;
        }

        // 2. Determine Weight Scale (Sw)
        size_t weight_size = totalInputFeatures * outputSize;
        fp32 max_weight = 0.0f;
        for (size_t i = 0; i < weight_size; i++) {
             fp32 abs_val = std::abs(getWeightData().get<fp32>(i));
             if (abs_val > max_weight) max_weight = abs_val;
        }
        if (max_weight < 1e-8f) max_weight = 1.0f;
        fp32 Sw = static_cast<fp32>(QUANT_MAX) / max_weight;
        ...
        // 4. Quantize Input
        std::vector<int8_t> input_q(totalInputFeatures);
        for (size_t i = 0; i < totalInputFeatures; i++) {
            i32 temp = static_cast<i32>(std::round(Si * dataIn.get<fp32>(i))) + zi;
            input_q[i] = static_cast<int8_t>(std::max<i32>(QUANT_MIN, std::min<i32>(QUANT_MAX, temp)));
        }

        // 5. Quantize Weights
        std::vector<int8_t> weight_q(weight_size);
        for (size_t i = 0; i < weight_size; i++) {
            i32 temp = static_cast<i32>(std::round(Sw * getWeightData().get<fp32>(i)));
            weight_q[i] = static_cast<int8_t>(std::max<i32>(QUANT_MIN, std::min<i32>(QUANT_MAX, temp)));
        }

        // 6. Quantize Biases
        std::vector<int32_t> bias_q(outputSize);
        for (size_t i = 0; i < outputSize; i++) {
            bias_q[i] = static_cast<i32>(std::round(Sb * getBiasData().get<fp32>(i)));
        }

        // 7. Hardware Execution
        // Dense layout: [Input, Output]
        // Hardware Stream: for each output neuron (f), stream all input features (c)
        
        std::vector<int32_t> output_acc(outputSize, 0);
        
        for (size_t f = 0; f < outputSize; f++) {
            
            // Set Packet Length: Cin * 1 * 1 ?
            // We are streaming Cin pairs.
            int packet_len = totalInputFeatures;
            Xil_Out32(FIFO_BASE_ADDR + XLLF_TLF_OFFSET, packet_len * 4);
            
            for (size_t c = 0; c < totalInputFeatures; c++) {
                int8_t act = input_q[c];
                // Weight index: c * outputSize + f
                int8_t weight = weight_q[c * outputSize + f];
            ...
            output_acc[f] = mac_result + bias_q[f];
        }

        // 8. Dequantize and ReLU
        float *outIdx = (float *)getOutputData().raw();
        for (size_t i = 0; i < outputSize; i++) {
            // Fix zero point correction for dequantization
            // result = (acc - zi * sum(weights)) / (Si * Sw)
            // Calculate weight sum for this neuron
            int32_t weight_sum = 0;
            for(size_t c=0; c<totalInputFeatures; c++) {
                weight_sum += weight_q[c * outputSize + i];
            }
            
            int32_t corrected_acc = output_acc[i] - (static_cast<i32>(zi) * weight_sum);
            float result = static_cast<float>(corrected_acc) / (Si * Sw);
